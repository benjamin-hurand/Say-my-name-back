...
public interface PersonEmailRepository extends JpaRepository<PersonEmailEntity, Long> {
  ...
  @Query("""
      select e
       ...
       where e.organizationId = :#{T(com.saymyname.core.multitenancy.TenantContext).get()}
  ...
  @Query(value = """
      SELECT pe.person_id AS personId,
       ...
       WHERE pe.organization_id = :#{T(com.saymyname.core.multitenancy.TenantContext).get()}
