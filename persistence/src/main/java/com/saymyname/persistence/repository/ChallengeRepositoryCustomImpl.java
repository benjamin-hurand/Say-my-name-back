...
public class ChallengeRepositoryCustomImpl implements ChallengeRepositoryCustom {
    ...
    public List<ChallengeCardProjection> findChallengeCards(ChallengeMenu challengeMenu) {
        ...
        // Jointures vers GameModeEntity, l'attribut de filtre et les versions du challenge
        ...
        // 1. Validité temporelle : versions et attributs doivent être valides à la date de début de saison
        if (challengeMenu.getSeasonStart() != null) {
            ...
            predicates.add(cb.or(cb.isNull(cvJoin.get("endDate")), cb.greaterThanOrEqualTo(cvJoin.get("endDate"), seasonStart)));
            // A quoi peuvent servir les predicates sur la validité des valeurs d'attributs ? pour l'instant on retire, on verra apres
            // predicates.add(cb.lessThanOrEqualTo(attrJoin.get("validFrom"), seasonStart));
            // predicates.add(cb.or(cb.isNull(attrJoin.get("validTo")), cb.greaterThanOrEqualTo(attrJoin.get("validTo"), seasonStart)));
        }

        // 2. Filtre de recherche textuelle sur la description du challenge et le titre du gamemode
        ...
        latestVersionSubquery.select(cb.max(cv2.get("versionNumber")))
            .where(
                ...
                cb.or(cb.isNull(cv2.get("endDate")), cb.greaterThanOrEqualTo(cv2.get("endDate"), challengeMenu.getSeasonStart()))
            );
        predicates.add(cb.equal(cvJoin.get("versionNumber"), latestVersionSubquery));

        // 7. Filtre sur le GameAttributeFilter par intersection (puisqu'un challenge n'a qu'un seul filtre)
        if (challengeMenu.getFilters() != null && challengeMenu.getFilters().getAttributeFilter() != null) {
            ...
            predicates.add(cb.equal(attrJoin.get("id"), challengeMenu.getFilters().getAttributeFilter().getAttribute().getId()));
            // Condition d'intersection entre la plage du challenge et la plage utilisateur :
            // (challenge.minFilterValue <= userFilter.maxValue) AND (challenge.maxFilterValue >= userFilter.minValue)
        ...
        // 8. Sous-requêtes pour le tri par popularité : compter le nombre de participants (tous utilisateurs) sur cette version
        ...
        // 9. Sous-requête pour compter les tentatives de l'utilisateur sur cette version
        ...
        // 10. Sous-requête pour obtenir le meilleur score de l'utilisateur sur cette version
        ...
        // 11. Définir la sous-requête pour obtenir le meilleur temps de l'utilisateur sur cette version
        ...
        // Utiliser unix_timestamp pour obtenir les secondes depuis l'époque, puis multiplier par 1000 pour obtenir des millisecondes
        Expression<Long> startTimestamp = cb.function("unix_timestamp", Long.class, attemptTimeForBest.get("attemptStart"));
        ...
        if (challengeMenu.getFilters() != null &&
            ...
            // PAS_COMMENCE : l'utilisateur n'a jamais tenté cette version (userAttempts = 0)
            ...
            // PAS_PARFAIT : l'utilisateur a tenté et son meilleur score est inférieur au nombre de questions
            ...
            // REUSSI : l'utilisateur a tenté et son meilleur score est égal au nombre de questions
            ...
            // PODIUM : l'utilisateur est en podium si le nombre de tentatives avec une performance meilleure
            // que la sienne (score supérieur, ou score égal et temps inférieur) est inférieur à 3
        ...
        bestAttemptStartSubquery.select(attemptStart.get("attemptStart"))
            .where(
                ...
                switch (sortCriterion.getSortType()) {
                    case CREATION_DATE:
                        orders.add(sortCriterion.getOrder() == OrderDirection.ASC ?
                            cb.asc(challenge.get("creationDate")) : cb.desc(challenge.get("creationDate")));
                        break;
                    case POPULARITY:
                        orders.add(sortCriterion.getOrder() == OrderDirection.ASC ?
                            cb.asc(cb.coalesce(participantsSubquery, 0L)) : cb.desc(cb.coalesce(participantsSubquery, 0L)));
                        break;
                    case LENGTH:
                        orders.add(sortCriterion.getOrder() == OrderDirection.ASC ?
                            cb.asc(cvJoin.get("questionCount")) : cb.desc(cvJoin.get("questionCount")));
                        break;
                    case PERFORMANCE:
                        ...
                        if (sortCriterion.getOrder() == OrderDirection.ASC) {
                            ...
                            orders.add(cb.desc(normalizedTime));         // Puis temps normalisé décroissant (temps court = mieux)
                        } else {
