...
import com.saymyname.persistence.mapper.tenant.TenantOrgEntityMapper;
import com.saymyname.persistence.repository.tenants.TenantOrgRepository;
...
public class TenantOrgDao {
        ...
        public TenantOrg update(TenantOrg org) {
                TenantOrgEntity e = repo.findById(org.getId())
                                .orElseThrow(() -> new NoSuchElementException(
                                                "Tenant org not found: id=" + org.getId()));
        ...
        public TenantOrg getById(Long id) {
                return mapper.toModel(repo.findById(id)
                                .orElseThrow(() -> new NoSuchElementException("Tenant org not found: id=" + id)));
        }

        public TenantOrg getByKey(String key) {
                return mapper.toModel(repo.findByOrgKey(key)
                                .orElseThrow(() -> new NoSuchElementException("Tenant org not found: key=" + key)));
        }
